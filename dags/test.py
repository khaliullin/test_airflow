...
with DAG(
    ...
    def prepare_df(data):
        ds = pd.json_normalize(data, sep="_")
    ...
    def postgres_load(data, schema, table, unique_key=None):
        db_string = os.environ['TEST_DATABASE_URL']
        db = create_engine(db_string)
        if unique_key:
            data.set_index(unique_key)
        data.to_sql(table, con=db, schema=schema, if_exists='replace', index=False)
        ...
        for t in tasks:
            ...
            if "data" in d and bool(d['data']) and len(d["data"][list_name]) > 0:
                ...
                try:
                    ...
                    postgres_load(data=data_df, schema="tiktok", table=table_name, unique_key=primary_key)
                    # comment upper line and uncomment this next 2 lines for Andrew
                        ...
                        if bool(childDataResponse['data']) and "mapname" in child:
                            ...
                            if mapname in childDataResponse["data"]:
                                ...
                                try:
                                    ...
                                    postgres_load(data=df_child, schema="tiktok", table=mapname, unique_key=primary_key_child)
                                    # comment upper line and uncomment this next 2 lines for Andrew
                                    # drop_table(table_name)
                                    # RedshiftLoad(data=df_child, schema="tiktok", table=mapname, unique_key=primary_key_child)
    ...
    tiktok_async_report = PythonOperator(
        ...
        python_callable=async_report_task,
        op_kwargs={
