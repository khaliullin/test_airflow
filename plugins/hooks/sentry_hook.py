...
SCOPE_TAGS = frozenset(
    ("task_id", "dag_id", "execution_date", "operator", "log_url", "max_tries", "try_number"))
SCOPE_TAGS_TASK = frozenset(
    ("downstream_task_ids", "upstream_task_ids"))
...
def add_tagging(task_instance):
    ...
    with configure_scope() as scope:
        ...
        for tag_name in SCOPE_TAGS_TASK:
            attribute = getattr(task_instance, "task")
            attribute_task = getattr(attribute, tag_name)
            scope.set_tag(tag_name, attribute_task)
...
def add_breadcrumbs(task_instance, session=None):
    ...
    task_instances = get_task_instances(
        dag_id, task_ids, execution_date, session)
...
def add_sentry(task_instance, *args, session=None, **kwargs):
    ...
    with push_scope():
        ...
        try:
            original_run_raw_task(task_instance, *args,
